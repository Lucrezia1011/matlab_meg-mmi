...
for nn = 1:N
    ...
    jj = jj +1;
     ...
     command_list{jj} =  sprintf(['export MCR_CACHE_ROOT=/lscratch/$SLURM_JOB_ID;'...
        ' cd /lscratch/$SLURM_JOBID; test -f mni_grid.txt || cp %s /lscratch/$SLURM_JOB_ID/;' ...
        ...
        gridname,meg_data_name,data_path,meg_data_name,...
...
file_handle = fopen(sprintf('mmi_LTA_powergrid_permute_%s.swarm',freq),'w+');
