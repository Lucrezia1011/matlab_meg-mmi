...
for sn = [1:6,8:9,14] % all subjects with continuos recordings
...
for ii = 1:length(L)
    ...
    if ~isempty(lf)
        ...
        R(1,k) = corr(mean(P{ii}(:,F>=1 & F<=4),2),mood{1});
        R(2,k) = corr(mean(P{ii}(:,F>=4 & F<=8),2),mood{1});
        R(3,k) = corr(mean(P{ii}(:,F>=8 & F<=13),2),mood{1});
        R(4,k) = corr(mean(P{ii}(:,F>=13 & F<=30),2),mood{1});
        R(5,k) = corr(mean(P{ii}(:,F>=30 & F<=55),2),mood{1});
        R(6,k) = corr(mean(P{ii}(:,F>=65 & F<=115),2),mood{1});
    end
