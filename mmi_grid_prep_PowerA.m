function mmi_grid_prep_PowerA(data_name,roiopt,gridres,freqband,filter_type)
% Anticipation period
...
data_filt = ft_preproc_bandpassfilter(data.trial{1}, data.fsample,freqband,filt_order,filter_type);
...
Cr = C + 0.01*E(1)*eye(size(C)); % 1% max singular value =~ 70*noise
...
save_name = sprintf('/data/MBDU/MEG_MMI3/results/mmiTrial_aal_prep_mu5max/pre_feedback/%.0f-%.0fHz_%s',...
...
% RPE(ttdel) = [];
E_LTA(ttdel) = [];
EltaH(ttdel) = []; % Expectation, defined by Hanna
% RltaH (ttdel) = [];

% refer to previous trial

% ltvmood = table(S,trials',mood',RPE,E_LTA,EltaH,RltaH,'VariableNames',...
%     {'subject','trial','mood','RPE','E','Elta','RPElta'});

ltvmood = table(S,trials',mood',E_LTA,EltaH,'VariableNames',...
    {'subject','trial','mood','Esum','E'});
