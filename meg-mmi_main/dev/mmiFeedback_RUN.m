...
for ii = 1:length(data_list)
    mmiFeedbackBF(data_list{ii},freqband,mu,gridres,windAct,windCont,plotopt)
end
...
Tstat = std(T,1); %39 subjects: had at least 20 negative RPE trials 
sourceant = [];
sourceant.pow = Tstat;
...
[~,ind] = min(sqrt(sum((sourceant.pos*10 - [14 50 16]).^2,2)));
figure;
histogram(T(:,ind))
title(sprintf('Peak Pseudo T-stat SAM image is %.2f +- %.2f\n',mean(T(:,ind)), std(T(:,ind))/sqrt(size(T,1))))
