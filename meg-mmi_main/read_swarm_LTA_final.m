...
clc
dx = 1;
for m = 1:3 %1:length(fit_parameters)
...
%         uplim =  snull(ceil(Nnull*(1-pv/2)));     
        ...
        if any(TFCE>uplim)
            ...
            fprintf('Peaks of %s:\n%s at %.0fms,  T=%.2f,  p-value=%.1e\n',fit_parameter,...
                    aal_labels{iir}, time(locs)*1e3, LME(locs),x.pValue(locs) )
        else
            locs = [];
            w = [];
        end
...
%           Highlight timepoints > TFCE null
        indp = find(TFCE > uplim | TFCE < lowlim);
...
%                 set(gcf,'color','w','name',fit_parameter,'position',[346  464  1097  399])
                set(gcf,'color','w','name',fit_parameter,'position',[322         404        1263         284])
            end
            n = n+1;
            sb = subplot(1,5,n);
            yyaxis left
...
legend('Average MEG signal','Contributing cluster','Fixed effect T-stat','TFCE significant')