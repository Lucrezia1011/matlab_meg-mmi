...
freqband = [25 40]; timew  = [0.3 0.5];
% freqband = [4 8];  timew = [0.2 0.6];
for ii = 14:length(data_list)
    data_name = data_list{ii};
    sub = data_name(5:9);
    mmiOutcomePower(data_name,roiopt,gridres,freqband,timew,mu); % mu = 0.05 
end

return

%% Find all common gradiometers: already done
...
for sn = 1:length(data_list) 
    ...
    if strcmp(roiopt,'grid')
        ...
        load(sprintf('outcome_grid_%.0f-%.0fHz_mu%g',...
...
%             freqband(1),freqband(2)))
    ...
    elseif  strcmp(roiopt,'sens')
        ...
        load(sprintf('outcome_sens_%.0f-%.0fHz',...
            freqband(1),freqband(2)))
    ...
    ltvout.recording = repmat(sn,size(ltvout,1),1);
    if isempty(ltvOut)
        ltvOut = ltvout;
    else
...
%% Write data (grid)
out_path = '/data/MBDU/MEG_MMI3/results/mmiTrial_grid/outcome_power';
...
writetable(ltvOut,[out_path,'/latent_vars.csv']);
 

%% Write data (sens)
out_path = '/data/MBDU/MEG_MMI3/results/mmiTrial_sens/outcome_power';
...
ltvOut = readtable([out_path,'/latent_vars.csv']);
...
for s = 1:length(subs)
    Ps(:,s) = mean(Pall(:,ltvOut.subject == subs(s)),2);
    recs = ltvOut.recording(ltvOut.subject == subs(s));
    nrecs = unique(recs);
...
fit_parameters = {'mood';'M';'E';'E_LTA';'E_sum';'RPE';'RPE_LTA';'RPE_sum'};
...
% plot the MEG data
ltvOut = readtable('/data/MBDU/MEG_MMI3/results/mmiTrial_sens/pre_mood/latent_vars.csv');
subs = unique(ltvOut.subject);
megz = zeros(size(meg,1),length(subs));
for ss = 1:length(subs)
    megm = mean(meg(:,ltvOut.subject==subs(ss)),2); % mean over trials
    megz(:,ss)  = zscore(megm);
end
