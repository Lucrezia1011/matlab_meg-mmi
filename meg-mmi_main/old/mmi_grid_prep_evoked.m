...
feedb_sample = outcome_match.sample(outcome_match.win~=0);
[datave,ttdel]= define_trials(feedb_sample, data, tasktime, [-.2,1],0);
ntrials = length(datave.trial);
datavem = cell2mat(datave.trial);

trials =  outcome_match.bv_index(outcome_match.win~=0)-12;
...
timew = datave.time{1}>=.200 & datave.time{1}<=.300; % FRN
S200300 = squeeze(mean(datas(:,timew,:),2));
...
for ii = 1:length(L)
    ...
    timew = datave.time{1}>=.200 & datave.time{1}<=.300; % FRN
    P200300{ii} = mean(ve(timew,:),1)*sign(c(ind));
...
Pin = cell2mat(P100150');
P100150 = zeros(size(grid.leadfield,2),size(Pin,2));
P100150(grid.inside,:) = Pin;
...
ltvmood = table(S,trials',mood',rpe,abs(rpe),RltaH,'VariableNames',...
    {'subject','trial','mood','RPE','RPE_abs','RPE_LTA'});

%%
save(save_name,'ltvmood','P100150','P175225','P200300','P225275','P250350','P300500',...
    'S100150','S175225','S200300','S225275','S250350','S300500');
...
for s = 1:length(param_list)
    ...
    load(save_name)
    r = r+1;
    ltvmood.recording = repmat(r,size(ltvmood,1),1);
    ltv = cat(1,ltv,ltvmood);
    ...
    s100150 = cat(2,s100150,S100150(ind,:));
...
dlmwrite([outpath,'BF_200-300ms_30Hzlowpass.txt'],p200300)
...
dlmwrite([outpath,'M_200-300ms_30Hzlowpass.txt'],s200300)
...
writetable(ltv,[outpath,'/latent_vars_FRN.csv']);
%% Plot sensors

ltv = readtable([outpath,'/latent_vars_FRN.csv']);

timew = {'100-150';'175-225';'225-275';'200-300';'250-350';'300-500'};
for ii = 5%1:6
s200300 = dlmread(sprintf('%sM_%sms_30Hzlowpass.txt',outpath,timew{ii}));

T.time{1} = [250];
T.trial{1} = mean(s200300(:,ltv.RPE_abs>median(ltv.RPE_abs)),2); 
T.sampleinfo = [1,1];
T.avg = T.trial;
T.label = sensall;
...
cfg.zlim = [-4 4]*1e-14;
ft_topoplotER(cfg, T)
title(sprintf('%sms |RPE|>median',timew{ii}))
...
T.trial{1} = mean(s200300(:,ltv.RPE_LTA>5),2) - mean(s200300(:,ltv.RPE_LTA<-2),2); 
T.avg = T.trial;
figure; set(gcf,'color','w');
cfg= [];
cfg.channel = sensall;
