...
[datave,ttdel]= define_trials(gamble_sample, data, tasktime, [1,4],0);
...
data.trial{1} = data_filt;
[datave,ttdel]= define_trials(gamble_sample, data, tasktime, [-3,5],0);
certain_sample = choice_match.sample(choice_match.choice==3);
[datavec,ttdel]= define_trials(certain_sample, data, tasktime, [1,5],0);

for tt = 1:length(datave.trial)
    ht = hilbert(datave.trial{tt}'); 
   datave.trial{tt} = abs(ht)';
end

for tt = 1:length(datavec.trial)
    ht = hilbert(datavec.trial{tt}'); 
   datavec.trial{tt} = abs(ht)';
end
...
cfg.foi        = [1:0.5:4,5:14,16:2:30];%,45:5:150];
cfg.toi        = 0:tstep:5; % previous step of 0.05s
cfg.keeptrials = 'no';
cfg.pad         = 'nextpow2';
cfg.taper = 'hanning';
cfg.t_ftimwin = ones(length(cfg.foi),1).*0.4;
cfg.trials = abs(RPE)>2;
TFRmult1 = ft_freqanalysis(cfg, datave);


cfg.trials = 'all';
cfg.toi   = 1:tstep:5;
base1 = ft_freqanalysis(cfg, datavec);  
base2 = zeros(length(base1.label),length(base1.freq));
for tt = 1:length(base1.freq)
    ind = squeeze(~isnan(base1.powspctrm(1,tt,:)));
    base2(:,tt) = mean(base1.powspctrm(:,tt,ind),3);
end
figure; 
ii = strncmp(data.label,'MRT',3);
pcolor(TFRmult1.time,TFRmult1.freq, squeeze(mean( ...
    (TFRmult1.powspctrm(ii,:,:)-base2(ii,:))./base2(ii,:)  ,1)));
shading flat;colorbar; caxis([-.4 .4])

timelockc = ft_timelockanalysis([],datavec);
base1 = mean(timelockc.avg(:,50:end-50),2);

figure; clf
hold on; ii = strncmp(data.label,'MRT',3);
cfg = []; cfg.trials = 1:23; 
timelock = ft_timelockanalysis(cfg,datave);
plot(timelock.time(50:end-50),mean(timelock.avg(ii,50:end-50)-base1(ii),1))

cfg = []; cfg.trials = 24:48; 
timelock = ft_timelockanalysis(cfg,datave);
plot(timelock.time(50:end-50),mean(timelock.avg(ii,50:end-50)-base1(ii),1))

cfg = []; cfg.trials = 49:67; 
timelock = ft_timelockanalysis(cfg,datave);
plot(timelock.time(50:end-50),mean(timelock.avg(ii,50:end-50)-base1(ii),1))
...
datapath = sprintf('/data/MBDU/MEG_MMI3/results/mmiTrial_aal_prep_mu5max/pre_feedback/M_anticipation_30Hzlowpass/');
nn =1;
cd([datapath,'lme_E'])
...
subplot(144)
T.avg = Teltasum; T.trial = Teltasum;
cfg = [];
cfg.channel = sensall;
cfg.layout = 'CTF275_helmet.mat';
cfg.zlim = [-4 4];
...
saveas(gcf,sprintf('~/matlab/figures/M_anticipation_30Hzlowpass.png',times{ii}))
