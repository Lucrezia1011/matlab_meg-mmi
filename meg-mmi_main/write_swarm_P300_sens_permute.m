...
d = dir(out_path);
N = 5000 - (length(d)-2);% make a command on a new line for each parameter
...
jobid = evalc(sprintf('!swarm --job-name %s_%s --gres lscratch:20 -g %s -t %s -b %s --time 4:00:00 --logdir %s -f permute_P300sens_%s.swarm --sbatch %s --devel',...
...
fprintf('swarm --job-name %s_%s --gres lscratch:20 -g %s -t %s -b %s --time 4:00:00 --logdir %s -f permute_P300sens_%s.swarm --sbatch %s\n',...
