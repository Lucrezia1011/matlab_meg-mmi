function optP = SAM_erf_costfun(angle, position, tanu, tanv, lf, covariance, inv_covariance, noise, noise_covariance, dataerf, timewind)
...
SAMweights_norm = SAMweights/sqrt( sum( (SAMweights*noise(1,1)).^2) );

ntrials = length(dataerf.time);
nsamples = length(dataerf.time{1});
dataloc = zeros(ntrials,nsamples);
for tt = 1:ntrials
    dataloc(tt,:) = SAMweights_norm*dataerf.trial{tt};
end

P_erf = abs(mean(dataloc,1));
