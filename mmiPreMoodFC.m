function mmiPreMoodFC(data_name,roiopt,gridres,freqband,mu)
...
% Post-hoc analysis: functional connectivity of whole brain with beta-gamma peak from PCC
...
lf = grid.leadfield{iim}; % Unit 1Am
...
%         w = Cr\lfo / (lfo'/Cr*lfo) ;
x = zeros(datave.sampleinfo(1,2),ntrials);
for tt = 1:ntrials
    x(:,tt) = w'*datave.trial{tt};
end
...
for ii = 1:length(L)
    ...
    for tt = 1:ntrials
        %             pp(tt) =  w'*Ctt(:,:,tt)*w;
        y= w'*datave.trial{tt};
        %             xt = leakage_reduction(x(:,tt),y');
        %             xt = xt - mean(xt);
        %             y = y - mean(y);
        %             env = abs(hilbert([xt,y']));
        %             aec(tt) = corr(env(11:end-10,1),env(11:end-10,2));
        %
        %             yt = leakage_reduction(y',x(:,tt));
        %             xt = x(:,tt) - mean(x(:,tt));
        %             yt = yt - mean(yt);
        %             env = abs(hilbert([xt,yt]));
        %             aec(tt) = (aec(tt) + corr(env(11:end-10,1),env(11:end-10,2)))/2;
        
        
        xt = x(:,tt) - mean(x(:,tt));
        yt = y' - mean(y);
        theta = angle(hilbert([xt,yt]));
        RP = diff(theta(11:end-10,:)'); % elimanet edges, inaccurate from hilbert
        % Find if node is phase locked with PCC over 3s period
        plv(tt) = mean(imag(exp(1i*RP))); % select imaginary part only to correct for leakage
        
    end
    % can do mean over windows or correlare with mood and expectation?
    % Do mean first: we would expect to see a correlation with
    % expectation anyway, since the PCC power is correlated with
    % expectation term (higher power --> higher SNR --> higher FC)
    PLV{ii} = plv;
    %         P{ii} = pp/(w'*noiseC*w);
    if mod(ii,300) == 0
        clc
...
P  = cell2mat(PLV);
% pc_m = corr(P', mood');
% pc_e = corr(P',LTAvars.E_LTA);
%%

save_name = sprintf('%s/pre_mood_PCCfc_%s_%.0f-%.0fHz_mu%.0f',...
...
% sourceant.inside = grid.inside; %sourcemolde.inside
% sourceant.pos = grid.pos; % sourcemodel.pos
% cfg = [];
% cfg.parameter = 'pow';
% sourceout_Int  = ft_sourceinterpolate(cfg, sourceant , mri); %mri_mni
% sourceout_Int.pow(~sourceout_Int.inside) = 0;
% sourceout_Int.coordsys = 'ctf'; %'mni'
% 
% 
% crang = [];
% cfg = [];
